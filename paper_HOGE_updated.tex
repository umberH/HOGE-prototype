...
For demonstration of the utility of HOGE, we prototype an end-to-end Explainable AI (XAI) pipeline for loan decisioning, integrating monotonic gradient boosting as ML models, local feature attribution with SHAP, and a finance domain aware Knowledge Graph (KG) to generate context-rich, human-centric explanations.
...
A gradient boosting model (XGBoost) with domain constraints was trained on structured financial data containing 500 loan applications to predict loan approval outcome. The pre-processed features used for model training include:
\begin{itemize}
...
We evaluate the HOGE prototype across two complementary axes: (A)~system and technical evaluation (quantitative), measuring explanation faithfulness, grounding precision, and graph retrieval accuracy; and (B)~human-centric evaluation (qualitative), assessing prediction--SHAP alignment, intermediate outcome traceability, and LLM grounding coverage. All evaluations were conducted on a random sample of 10~applications drawn from the 500-application dataset. Results for both evaluation axes are summarized in Table~\ref{tab:evalSummary}.
...
To measure whether HOGE explanations faithfully reflect the model's actual reasoning, we employed perturbation tests. For each sampled application, we identified the top SHAP feature, artificially perturbed it (e.g., flipping \texttt{Credit\_History} from 1 to 0, or reducing \texttt{CoapplicantIncome} to 20\% of its original value), re-computed SHAP values on the perturbed input, and evaluated whether the model's probability shift and new top driver were consistent with the perturbation.

An LLM-as-judge (GPT-4o) scored each perturbation test on a 0--1 scale across three criteria: whether the perturbation was reflected in the probability shift, whether the shift direction was correct, and whether the top driver changed appropriately.
...
For each sampled application, we generated a full LLM explanation via HOGE, then employed an LLM-as-judge to score every factual claim in the narrative against the ground-truth KG evidence. The judge evaluated three HOGE Level~1 metrics:
...
To validate that the KG correctly stores and retrieves the right SHAP contributions and policy rules for each application, we compared KG query results against the ground-truth SHAP CSV files. For each application, we measured:
...
For each sampled application, we verified that the model's prediction direction (approved = positive, declined = negative) was consistent with the net sum of SHAP values. All 10~applications showed perfect alignment: approved applications had positive net SHAP sums (range: 0.003 to 2.327), and declined applications had negative sums ($-$4.065 and $-$5.176). All 10~predictions were also correct against the ground-truth labels, achieving 100\% accuracy on the sample.

\subsubsection{Intermediate Outcome Traceability.}

A core claim of HOGE is that every stage of the pipeline---from raw input features through engineered features, model prediction, SHAP attribution, policy evaluation, KG retrieval, and LLM generation---produces traceable intermediate outcomes. We validated this by collecting all eight stages for each application and recording them both in JSON and a multi-sheet Excel workbook.
...
To verify that the LLM uses the intermediate outcomes in its explanations, we performed an automated cross-check. For each application, we checked whether each raw feature name, each top-5 SHAP feature, and each violated policy rule appeared in the LLM narrative or the LLM's self-reported feature list.
