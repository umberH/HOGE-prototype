...
%
\authorrunning{Murshed and Bandara}   % abbreviated author list (for running head)
%
...
The HOGE artifact is demonstrated through a loan decisioning case study (Section~5), in which we instantiate the full pipeline---from predictive model training through SHAP attribution, KG construction, and LLM narrative generation---for 500~loan applications. The demonstration is operationalised via three scenarios (prediction retrieval, narrative explanation, and policy validation) and evaluated using a metric suite aligned with interpretability evaluation guidance~\cite{doshi2017}.
...
The current prototype evaluation fully operationalises Level~1 metrics (SF, EC, HRC) through automated evaluation, and partially addresses Level~2 through automated consistency and traceability checks. Full human-subject evaluation with Comprehension Score and Trust Calibration Error (Level~2) and application-grounded evaluation with Decision Utility Gain and Actionability Index (Level~3) are proposed for future validation studies.


\subsection{Stage 6- Communication}
This paper constitutes the primary communication artefact for the HOGE framework, presenting the design rationale, architectural specification, prototype implementation, and empirical evaluation. The open-source prototype repository\footnote{\url{https://github.com/staq-6/Phd-Experiment1}} provides all source code, evaluation scripts, output artefacts (JSON explanations, evaluation workbooks), and reproducibility instructions, enabling independent verification and extension of the reported results.
...
Figure~\ref{fig:kg_schema} shows the instantiated ontology schema as realised in the Neo4j knowledge graph, depicting the nine node labels (Applicant, LoanApplication, Feature, FeatureValue, ModelExplanation, FeatureContribution, PolicyRule, RiskFactor, CounterfactualScenario) and their directed relationship types.
...
\begin{enumerate}
    \item Creates constraints (uniqueness on application IDs, feature names, explanation IDs, counterfactual IDs)
    ...
    \item Loads counterfactual scenarios and links them to applications via \texttt{HAS\_COUNTERFACTUAL} and to the perturbed feature via \texttt{PERTURBS\_FEATURE}
\end{enumerate}

Each evidence item is persisted with an explicit KG path (e.g., \texttt{(LoanApplication:LP001006) -[:HAS\_SHAP\_EXPLANATION]-> (ModelExplanation) -[:HAS\_CONTRIBUTION]-> (FeatureContribution) -[:FOR\_FEATURE]-> (Feature:CoapplicantIncome)}), enabling full provenance traceability from any narrative claim back to its source in the graph. Counterfactual scenarios are similarly persisted with the path \texttt{(LoanApplication) -[:HAS\_COUNTERFACTUAL]-> (CounterfactualScenario) -[:PERTURBS\_FEATURE]-> (Feature)}, ensuring that contrastive what-if claims are also KG-grounded.
...
\begin{itemize}
    ...
    \item A structured JSON output format requiring: summary, positive drivers, negative drivers, policy violations, recommendation, counterfactual what-if scenario, full narrative, and a list of features used
\end{itemize}
...
\begin{itemize}
    ...
    \item counterfactual what-if: the minimal actionable change that would flip the decision
...
\begin{itemize}
    ...
    \item \textbf{51}~CounterfactualScenario nodes (for 10 evaluation applications) linked via \texttt{HAS\_COUNTERFACTUAL} to applications and via \texttt{PERTURBS\_FEATURE} to actionable features
\end{itemize}
...
\begin{itemize}
    ...
    \item ``What minimal change to Application~X would flip the decision?''
\end{itemize}
...
\begin{table}[htp]
    ...
    \begin{tabular}{l l r}
        ...
        \midrule
        \multirow{2}{*}{C. Counterfactual}
            & Single-Feature Flip Rate & 9/10 (90\%) \\
            & Most Common Flip Feature & Credit\_History \\
        \bottomrule
    \end{tabular}
\end{table}
...
A core claim of HOGE is that every stage of the pipeline produces traceable intermediate outcomes. To validate this, for each sampled application we collect and record the outputs of all eight pipeline stages:
...
\begin{enumerate}
    ...
    \item \textbf{Stage~6 --- Counterfactual Analysis}: Minimal actionable feature changes that would flip the prediction, persisted as \texttt{CounterfactualScenario} nodes in the KG.
    \item \textbf{Stage~7 --- KG Retrieval}: Confirmation that all features, contributions, policy links, and counterfactual scenarios were retrieved from Neo4j.
    \item \textbf{Stage~8 --- LLM Generation}: The structured JSON explanation with narrative, drivers, violations, counterfactual what-if, and provenance metadata.
\end{enumerate}
...
\begin{table}[htp]
    ...
    \begin{tabular}{l l l}
        ...
        \midrule
        6. Counterfactual & Minimal flip & CoapplicantInc. +\$5000 \\
         & New probability & 0.829 (Approved) \\
        \midrule
        7. KG & Status & Retrieved \\
        8. LLM & Status & Generated \\
        \bottomrule
    \end{tabular}
\end{table}
...
\subsection{Counterfactual Analysis Results}
\label{sec:counterfactual}

To address the contrastive property of human explanations~\cite{miller2019} (``Why outcome~A instead of~B?''), HOGE includes a counterfactual explanation module that computes the minimal actionable feature changes needed to flip a prediction. Counterfactual results are persisted as \texttt{CounterfactualScenario} nodes in the KG, linked to the application via \texttt{HAS\_COUNTERFACTUAL} and to the perturbed feature via \texttt{PERTURBS\_FEATURE}. This ensures that contrastive what-if claims in the LLM narrative are governed by the same semantic contract as all other explanation claims---the LLM retrieves counterfactual evidence from the KG rather than computing it at inference time.

Results are presented in Table~\ref{tab:counterfactual}. A single-feature flip was found for 9 out of 10~applications (90\%). Credit\_History was the most powerful flip lever: flipping it reversed the decision in 7~cases, with probability shifts exceeding 0.75 in most instances. For the two declined applications, the module identified actionable paths to approval: LP001250 could flip by adding a co-applicant income of \$5,000 (probability: 3.6\%~$\to$~82.9\%), and LP002446 could flip by establishing positive credit history (probability: 1.2\%~$\to$~87.0\%).

The single failure case (LP001238, Approved at 95.7\%) could not be flipped by any single actionable feature change, indicating a robust approval that is resilient across the actionable feature space---a useful signal for a loan officer confirming the strength of the decision.

\begin{table}[htp]
    \centering
    \caption{Counterfactual analysis: minimal single-feature changes to flip predictions}
    \label{tab:counterfactual}
    \begin{tabular}{l l r l l r}
        \toprule
        \textbf{App ID} & \textbf{Current} & \textbf{Prob} & \textbf{Feature} & \textbf{Change} & \textbf{New Prob} \\
        \midrule
        LP002170 & Approved & 0.944 & Credit\_History & Flip 1$\to$0 & 0.176 \\
        LP001250 & Declined & 0.036 & CoapplicantInc. & Add \$5000 & 0.829 \\
        LP002209 & Approved & 0.940 & Credit\_History & Flip 1$\to$0 & 0.054 \\
        LP001536 & Approved & 0.781 & Loan\_Amt\_Term & Extend to 360 & 0.343 \\
        LP001357 & Approved & 0.817 & Credit\_History & Flip 1$\to$0 & 0.017 \\
        LP002266 & Approved & 0.905 & Credit\_History & Flip 1$\to$0 & 0.024 \\
        LP002223 & Approved & 0.881 & Credit\_History & Flip 1$\to$0 & 0.036 \\
        LP001439 & Approved & 0.686 & Credit\_History & Flip 1$\to$0 & 0.015 \\
        LP001238 & Approved & 0.957 & --- & No single flip & --- \\
        LP002446 & Declined & 0.012 & Credit\_History & Flip 0$\to$1 & 0.870 \\
        \midrule
        \multicolumn{5}{l}{\textbf{Flip success rate}} & \textbf{9/10} \\
        \bottomrule
    \end{tabular}
\end{table}

These counterfactual explanations are integrated into the LLM narrative via the structured prompt, enabling the generated explanation to include contrastive what-if statements grounded in actual model behaviour rather than LLM speculation. Because counterfactual scenarios are persisted in the KG with explicit provenance paths, the evidence bundle for each explanation includes entries of the form \texttt{(LoanApplication) -[:HAS\_COUNTERFACTUAL]-> (CounterfactualScenario) -[:PERTURBS\_FEATURE]-> (Feature)}, maintaining end-to-end traceability. This directly operationalises Miller's contrastive property~\cite{miller2019} within the HOGE framework.
...
The KG achieved Precision@K~=~1.0 at all K values and 100\% prediction match, confirming lossless provenance. This contrasts with systems where retrieval noise propagates into explanations---Xu et al.~\cite{synkg2025} noted that LLM-generated triples can introduce noise despite filtering. HOGE avoids this by treating the KG as a faithful mirror of ground-truth SHAP data rather than an LLM-expanded knowledge store. Additionally, counterfactual scenarios are persisted as \texttt{CounterfactualScenario} nodes in the same KG, linked via \texttt{HAS\_COUNTERFACTUAL} and \texttt{PERTURBS\_FEATURE} relationships, extending provenance coverage to contrastive claims.
...
The Knowledge Graph adds four critical advantages beyond traditional ML explainability:
\begin{enumerate}
    ...
    \item \textbf{Counterfactual Provenance}: Counterfactual what-if scenarios are persisted as first-class KG nodes, ensuring that contrastive claims in the LLM narrative are governed by the semantic contract rather than generated ad hoc.
\end{enumerate}
